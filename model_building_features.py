...
# This code executes with different levels of randomly selected features per bootstrap
# These predictions are saved in./predictions/feature_results_X
import glob
import numpy as np
import os
...
n_features_list = [10, 25, 50, 100, 250, 500]
for n_features in n_features_list:
    if os.path.exists('./predictions') == False:
        os.mkdir('./predictions')
    if os.path.exists('./predictions/feature_results_{}'.format(str(n_features))) == False:
        os.mkdir('./predictions/feature_results_{}'.format(str(n_features)))
    i = 0
    ...
    pd.concat(logistic_regression, axis = 1).to_csv('./predictions/feature_results_{}/lr_predictions.csv'.format(n_features), index = False)
    pd.concat(random_forest, axis = 1).to_csv('./predictions/feature_results_{}/rf_predictions.csv'.format(n_features), index = False)
    pd.concat(support_vector, axis = 1).to_csv('./predictions/feature_results_{}/sv_predictions.csv'.format(n_features), index = False)
    pd.concat(neural_network, axis = 1).to_csv('./predictions/feature_results_{}/nn_predictions.csv'.format(n_features), index = False)
    pd.concat(ensemble_classifier, axis = 1).to_csv('./predictions/feature_results_{}/ec_predictions.csv'.format(n_features), index = False)