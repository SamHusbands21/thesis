...
# This code executes with differnt levels of hyperparameter searching per ML algorithm
# These predictions are saved in./predictions/h_results_X
import glob
import numpy as np
import os
...
all_filenames = glob.glob('./data/hp_bootstraps/*.{}'.format('csv'))
all_filenames = [s.replace('\\', '/') for s in all_filenames]

h_searches = [10, 20, 30, 40, 50]
for h_search in h_searches:
    if os.path.exists('./predictions') == False:
        os.mkdir('./predictions')
    if os.path.exists('./predictions/h_results_{}'.format(h_search)) == False:
        os.mkdir('./predictions/h_results_{}'.format(h_search))
    ...
    for filename in all_filenames:
        ...
        mf, map  = pd.read_csv(filename).drop('id', axis = 1), pd.read_csv(filename)['id']
    ...
    pd.concat(logistic_regression, axis = 1).to_csv('./predictions/h_results_{}/lr_predictions.csv'.format(h_search), index = False)
    pd.concat(random_forest, axis = 1).to_csv('./predictions/h_results_{}/rf_predictions.csv'.format(h_search), index = False)
    pd.concat(support_vector, axis = 1).to_csv('./predictions/h_results_{}/sv_predictions.csv'.format(h_search), index = False)
    pd.concat(neural_network, axis = 1).to_csv('./predictions/h_results_{}/nn_predictions.csv'.format(h_search), index = False)
    pd.concat(ensemble_classifier, axis = 1).to_csv('./predictions/h_results_{}/ec_predictions.csv'.format(h_search), index = False)
