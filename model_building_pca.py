...
# This code executes with PCA applied to the training data
# These predictions are saved in./predictions/pca_results
import glob
import numpy as np
import os
...
def hyp_tuner_rf(md_range,
    ...
    train_df.set_index('SEQN',drop = True, inplace = True)
    test_df = test_df.merge(mapper, left_index = True, right_index = True)
    test_df.set_index('SEQN',drop = True, inplace = True)
...
all_filenames = glob.glob('./data/large_bootstraps/*.{}'.format('csv'))
all_filenames = [s.replace('\\', '/') for s in all_filenames]
if os.path.exists('./predictions') == False:
    os.mkdir('./predictions')
if os.path.exists('./predictions/pca_results') == False:
    os.mkdir('./predictions/pca_results')
i = 0
...
for filename in all_filenames:
    ...
    mf, mapp  = pd.read_csv(filename).drop('SEQN', axis = 1), pd.read_csv(filename)['SEQN']
    #implementing the PCA
    X = pd.read_csv(filename).drop('deceased', axis=1).set_index('SEQN')
    y = pd.read_csv(filename).set_index('SEQN')['deceased']
    #scaling
    ...
    variation.append(pca.explained_variance_ratio_.cumsum())
...
variation_csv = pd.DataFrame(variation)
variation_csv.to_csv('./predictions/pca_results/scree.csv', index = False)
pd.concat(logistic_regression, axis = 1).to_csv('./predictions/pca_results/lr_predictions.csv', index = False)
pd.concat(random_forest, axis = 1).to_csv('./predictions/pca_results/rf_predictions.csv', index = False)
pd.concat(support_vector, axis = 1).to_csv('./predictions/pca_results/sv_predictions.csv', index = False)
pd.concat(neural_network, axis = 1).to_csv('./predictions/pca_results/nn_predictions.csv', index = False)
pd.concat(ensemble_classifier, axis = 1).to_csv('./predictions/pca_results/ec_predictions.csv', index = False)